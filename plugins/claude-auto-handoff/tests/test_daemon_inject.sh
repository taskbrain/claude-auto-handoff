...
run_inject() {  # <home> <submit_after> <reenter_max> <sendfail> <capfail> [pending_screen] [pretext_screen] [compaction_command] [action]
    local home="$1"
    SEND_LOG="${home}/send.log"; ENTER_CNT="${home}/enter.cnt"; : > "${SEND_LOG}"; : > "${ENTER_CNT}"
    rm -f "${home}/text_sent"
    # arg8 = COMPACTION_COMMAND (空なら未設定 = inject 既定 /compact)。/clear フォールバックや不正値矯正の検証用。
    # arg9 = inject に渡す action (既定 COMPACT)。COMPACT_RECOVER (orphan-400 回復) の /clear 強制検証用。
    PATH="${TMOCK}:${PATH}" SEND_LOG="${SEND_LOG}" ENTER_CNT="${ENTER_CNT}" TEXT_SENT="${home}/text_sent" \
        ...
        SUBMIT_AFTER="$2" SENDFAIL="${4:-0}" CAPFAIL="${5:-0}" REENTER="$3" COMPACTION_COMMAND="${8:-}" \
        INJECT_ACTION="${9:-COMPACT}" bash -c '
            . "'"${DAEMON}"'" --source-only
...
# 8) [送出順序] 入力欄 clear (C-u) → 圧縮コマンド → 継続プロンプト本文 → 単独 Enter。
#    圧縮コマンドの直前に C-u を送ることで、入力欄に下書きが残っていても "${cmd}" が連結されず slash
#    command として実行される (Codex I1/C2 根治の回帰ガード)。
T="$(mktemp -d)"; run_inject "${T}" 1 5 0 0 >/dev/null
assert_eq "$(sed -n '1p' "${T}/send.log")" "-t %5 C-u" "1 行目 = 入力欄 clear (C-u, 圧縮コマンド直前)"
assert_eq "$(sed -n '2p' "${T}/send.log")" "-t %5 /compact Enter" "2 行目 = /compact Enter"
assert_eq "$(sed -n '3p' "${T}/send.log" | grep -cF "${MARK}")" "1" "3 行目 = 継続プロンプト本文"
...
# 13) [/clear フォールバック経路] COMPACTION_COMMAND=/clear → 1 行目は /clear Enter (1M で /compact 不発時の
#     運用フォールバック。CLAUDE.md 記載の経路を daemon レベルで回帰固定)。
T="$(mktemp -d)"; run_inject "${T}" 1 5 0 0 "" "" /clear >/dev/null
assert_eq "$(sed -n '1p' "${T}/send.log")" "-t %5 C-u" "/clear 経路も 1 行目は C-u (入力欄 clear)"
assert_eq "$(sed -n '2p' "${T}/send.log")" "-t %5 /clear Enter" "COMPACTION_COMMAND=/clear → 2 行目 /clear Enter (運用フォールバック経路)"

# 14) [不正値の矯正] COMPACTION_COMMAND=/bogus → /compact に矯正して送出 + stderr に警告 (ライブ pane への
#     不正文字列 literal 送出を防ぐ inject の値検証 case を回帰固定)。
T="$(mktemp -d)"; run_inject "${T}" 1 5 0 0 "" "" /bogus 2>"${T}/err.log" >/dev/null
assert_eq "$(sed -n '2p' "${T}/send.log")" "-t %5 /compact Enter" "不正 COMPACTION_COMMAND → /compact に矯正して送出 (1行目はC-u)"
assert_eq "$(grep -cF '無効な COMPACTION_COMMAND' "${T}/err.log")" "1" "不正値は stderr に警告を出す"

# 15) [P1-400 stall ループ修正] action=COMPACT_RECOVER (orphan-400 検出時) は COMPACTION_COMMAND に
#     依らず /clear を強制する。orphan-400 (#40305) は in-place /compact では復旧不能 (要約 API 呼出
#     自体が壊れた tool_result で 400 になり /compact 再送でも回復しない = 400→/compact→400 の stall
#     ループ)。/clear のみが壊れた履歴を捨てて確実に回復する。COMPACTION_COMMAND 既定 (/compact) でも
#     COMPACT_RECOVER は /clear を撃つこと。
T="$(mktemp -d)"; run_inject "${T}" 1 5 0 0 "" "" "" COMPACT_RECOVER >/dev/null
assert_eq "$(sed -n '1p' "${T}/send.log")" "-t %5 C-u" "COMPACT_RECOVER も 1 行目は C-u (入力欄 clear)"
assert_eq "$(sed -n '2p' "${T}/send.log")" "-t %5 /clear Enter" "COMPACT_RECOVER → COMPACTION_COMMAND 既定でも /clear 強制 (orphan-400 回復)"
assert_eq "$(grep -cF '/compact' "${T}/send.log")" "0" "COMPACT_RECOVER は /compact を一切送らない (400→/compactループ回避)"

# 16) [P1-400] COMPACTION_COMMAND=/compact を明示しても COMPACT_RECOVER は /clear を強制する
T="$(mktemp -d)"; run_inject "${T}" 1 5 0 0 "" "" /compact COMPACT_RECOVER >/dev/null
assert_eq "$(sed -n '2p' "${T}/send.log")" "-t %5 /clear Enter" "COMPACTION_COMMAND=/compact でも COMPACT_RECOVER は /clear 強制"

# 17) [境界固定] action=COMPACT (通常圧縮) は従来どおり COMPACTION_COMMAND (既定 /compact) を使う
T="$(mktemp -d)"; run_inject "${T}" 1 5 0 0 "" "" "" COMPACT >/dev/null
assert_eq "$(sed -n '2p' "${T}/send.log")" "-t %5 /compact Enter" "COMPACT (通常) → 既定 /compact のまま (非回帰)"
