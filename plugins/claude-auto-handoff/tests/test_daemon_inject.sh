...
run_inject() {  # <home> <submit_after> <reenter_max> <sendfail> <capfail> [pending_screen] [pretext_screen] [compaction_command]
    local home="$1"
    ...
    PATH="${TMOCK}:${PATH}" SEND_LOG="${SEND_LOG}" ENTER_CNT="${ENTER_CNT}" TEXT_SENT="${home}/text_sent" \
        ...
        SUBMIT_AFTER="$2" SENDFAIL="${4:-0}" CAPFAIL="${5:-0}" REENTER="$3" COMPACTION_COMMAND="${8:-}" bash -c '
            . "'"${DAEMON}"'" --source-only
