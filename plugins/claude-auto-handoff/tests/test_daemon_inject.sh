...
# 13) [/clear フォールバック経路] COMPACTION_COMMAND=/clear → 1 行目は /clear Enter (1M で /compact
#     不発時の運用フォールバックを daemon レベルで回帰固定)。
T="$(mktemp -d)"; run_inject "${T}" 1 5 0 0 "" "" /clear >/dev/null
assert_eq "$(sed -n '1p' "${T}/send.log")" "-t %5 /clear Enter" "COMPACTION_COMMAND=/clear → /clear Enter を送出 (運用フォールバック経路)"
...
assert_eq "$(sed -n '1p' "${T}/send.log")" "-t %5 /compact Enter" "不正 COMPACTION_COMMAND → /compact に矯正して送出"
