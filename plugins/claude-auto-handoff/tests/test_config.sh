...
unset PREPARE_PCT COMPACT_PCT CRITICAL_PCT DEBOUNCE_SECONDS PREPARE_PROMPT_DEBOUNCE_SECONDS \
      ...
      RESUME_DELAY_SECONDS RESUME_MAX_WAIT_SECONDS COMPACTION_COMMAND COMPACTED_TTL_SECONDS \
      RESUME_REENTER_MAX STATE_FRESH_SECONDS HANDOFF_RESTORE_FRESH_SECONDS \
...
# Part B: 圧縮完了 poll の上限 (/compact 要約は /clear より長めの 90) + 継続プロンプト Enter 再確定の最大回数
assert_eq "${RESUME_MAX_WAIT_SECONDS}" "90" "RESUME_MAX_WAIT_SECONDS 既定 90"
assert_eq "${RESUME_REENTER_MAX}" "3" "RESUME_REENTER_MAX 既定 3"
