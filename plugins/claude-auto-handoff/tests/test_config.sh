...
unset PREPARE_PCT COMPACT_PCT CRITICAL_PCT DEBOUNCE_SECONDS PREPARE_PROMPT_DEBOUNCE_SECONDS \
      ...
      HANDOFF_RESTORE_FRESH_SECONDS NEXT_STEPS_MIN_CHARS \
...
# P1b: NEXT STEPS 薄さ警告 (Stage3 additionalContext) の閾値 (非空白文字数)
assert_eq "${NEXT_STEPS_MIN_CHARS}" "40" "NEXT_STEPS_MIN_CHARS 既定 40 (P1b 薄い NEXT STEPS 判定の下限)"
# A2: 復元の鮮度ゲート
