...
# 4) native installer は pane_current_command にバージョン文字列 (例 2.1.161) を返す。bash pane が
#    先頭に並んでも version 文字列の claude pane を優先する。
export PANES="%5 ${T} bash
%3 ${T} 2.1.161"
assert_eq "$(resolve_pane_by_cwd "${T}")" "%3" "native-install の version 文字列 pane を claude として優先 (bash 先頭でも)"
# 5) cc / bun ランチャ名も claude として優先する
export PANES="%5 ${T} bash
%7 ${T} cc"
assert_eq "$(resolve_pane_by_cwd "${T}")" "%7" "cc ランチャ名も claude として優先"
