...
- **継続プロンプトの確実な自動送信**: 圧縮後に入力欄へ投入する継続プロンプトは、テキストと Enter を
  分離して送る (長い多バイトテキスト + Enter を同一 send-keys で送ると端末が bracketed-paste と解釈し
  末尾 Enter を吸収して未送信になるため)。入力欄 (罫線間) にプロンプトが残る間は bounded に Enter を
  再送して送信を確定する。
- **誤送信防止**: 圧縮直後の窓でユーザーが入力欄へ自分のテキストを打っていた場合 (先行入力 / 追記 /
  置換) は、それを積極検出できたときだけ Enter を抑止し、ユーザーの打ちかけを誤送信しない。検出不能
  (capture 不能 / 入力欄特定不能 / プロンプト部分着地) では送信を優先する (未送信で停止するより送る)。
